view: delivery_project_cycle_times_hkm {
  derived_table: {
      sql: WITH base AS (
        SELECT
          issue_key,
          ...
          issue_created_at
        FROM `ra-development.fivetran_jira_hkm_jira.jira__daily_sprint_issue_history`
        WHERE issue_key LIKE 'DA-%'
      ),
      ...
      first_status AS (
        ...
        FROM (
          ...
          FROM `ra-development.fivetran_jira_hkm_jira.jira__daily_sprint_issue_history` d
          WHERE d.issue_key LIKE 'DA-%'
        )
        WHERE rn = 1
      ),
      ...
      issue_meta AS (
        SELECT
          ...
          pe.project_name,
          ie.assignee_email
        FROM `ra-development.fivetran_jira_hkm_jira.jira__issue_enhanced` ie
        LEFT JOIN `ra-development.fivetran_jira_hkm_jira.jira__project_enhanced` pe
          ON ie.project_id = pe.project_id
      )

      SELECT
        'Hunkemoller' as client_name,
        l.issue_key,
        l.issue_id,
        l.assignee_name,
        l.sprint_id,
        ...
        CASE
          ...
          ELSE COALESCE(DATE_DIFF(cc.cycle_complete_day, cs.cycle_start_day, DAY),1)
        ...
        CASE WHEN cycle_start_day is not null and cycle_complete_day is not null then 1 else 0 end as is_complete_issue,
